...
export default function DesktopReviewInterface({
  ...
  onVideoChange,
  notes,
  existingNotes = [], // הערות קיימות מהמסד נתונים
  onAddNote,
  onRemoveNote,
  onSubmitNotes,
  onApproveVideo
  ...
  const getExistingVideoNotes = () => {
    if (!currentVideo?.currentVersion) return [];
    return existingNotes.filter(note => note.video_version_id === currentVideo.currentVersion.id);
  };
                  ...
                  <div className="flex-1 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="font-medium text-slate-200">
                        Notes ({getVideoNotes().length + getExistingVideoNotes().length})
                      </h4>
                      {notes.length > 0 && (
                        <Button
                    ...
                    <ScrollArea className="flex-1">
                      <div className="space-y-3">
                        ...
                        {getExistingVideoNotes().length > 0 && (
                          <>
                            <div className="flex items-center gap-2 mb-3">
                              <div className="h-px bg-slate-600 flex-1"></div>
                              <span className="text-xs text-slate-400 px-2">Previous Feedback</span>
                              <div className="h-px bg-slate-600 flex-1"></div>
                            </div>
                            {getExistingVideoNotes().map((note) => (
                              <div
                            ...
                            {getVideoNotes().length > 0 && (
                              <div className="flex items-center gap-2 my-3">
                                <div className="h-px bg-slate-600 flex-1"></div>
                                <span className="text-xs text-slate-400 px-2">New Notes</span>
                                <div className="h-px bg-slate-600 flex-1"></div>
                              </div>
                            )}
                          </>
